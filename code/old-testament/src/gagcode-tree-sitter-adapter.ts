...
import { emptyFactBag, GagcodeIdFactory, type GagcodeFactBag, type GagcodeSyntaxAdapter } from "./gagcode-adapters.js";
...
const TREE_SITTER_LANGUAGES: Array<{
  extensions: string[];
...
export function createTreeSitterSyntaxAdapter(idFactory: GagcodeIdFactory): GagcodeSyntaxAdapter {
  return {
    id: "tree-sitter",
    supports(relativePath: string): boolean {
      return Boolean(languageFor(relativePath));
    },
    analyzeFile(input): GagcodeFactBag {
...
function isMemberAccess(node: SyntaxNode): boolean {
...
function symbolKind(node: SyntaxNode): "function" | "class" | "type" | "interface" | "enum" | "constant" | "method" | "export" {
  ...
  if (node.type.includes("lexical") || node.type.includes("variable")) return "constant";
  return "function";
}

function typeKind(node: SyntaxNode): "type" | "interface" | "enum" | "class" {
  if (node.type.includes("interface")) return "interface";
  if (node.type.includes("enum")) return "enum";
  if (node.type.includes("class")) return "class";
  return "type";
...
function importSource(node: SyntaxNode): string {
...
function entryKindForCall(callee: string, node: SyntaxNode): "http-route" | "ui-action" | "cli-command" | "test" | undefined {
  if (/^(app|router)\.(get|post|put|patch|delete)$/i.test(callee)) return "http-route";
  if (/^(describe|it|test)$/i.test(callee)) return "test";
  if (/^(program|command)\.?/.test(callee) || /\.command$/.test(callee)) return "cli-command";
  if (node.text.includes("onClick") || node.text.includes("onSubmit")) return "ui-action";
  return undefined;
}

function memberAccess(node: SyntaxNode): { object?: string; field: string } | undefined {
...
function literalValues(node: SyntaxNode): string[] | undefined {
  const values = node
