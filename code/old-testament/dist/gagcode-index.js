...
export function queryStructuredIndex(index, query, limit) {
...
export function queryVectorIndex(index, query, limit) {
...
export function queryGraphIndex(index, query, depth, limit) {
    const terms = tokenize(query);
...
function buildStructuredIndex(facts, generatedAt) {
    ...
    for (const fact of facts) {
        ...
        if (["symbol", "definition", "reference", "call"].includes(fact.kind))
            add(index.bySymbol, fact.label, fact.id);
        if (fact.kind === "fieldRead" || fact.kind === "fieldWrite")
            add(index.byField, fact.label, fact.id);
        if (fact.kind === "type")
            add(index.byType, fact.label, fact.id);
        if (fact.kind === "entry")
            add(index.byEntryKind, fact.label, fact.id);
    }
    return index;
}
...
function buildVectorIndex(facts, generatedAt) {
...
function addMatchingKeys(index, term, output) {
...
function weightsFor(terms, vocabulary, documentFrequency, documentCount) {
...
function factKindToNodeKind(kind) {
    if (kind === "entry")
        return "entry";
    if (kind === "symbol" || kind === "definition" || kind === "reference" || kind === "call")
        return "symbol";
    if (kind === "type")
        return "type";
