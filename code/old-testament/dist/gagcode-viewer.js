...
export function renderGagcodeHtml(model) {
    ...
    const entries = model.facts.entries
        .slice(0, 50)
        .map((entry) => `<tr><td>${escapeHtml(entry.kind)}</td><td>${escapeHtml(entry.name)}</td><td>${escapeHtml(`${entry.file}:${entry.line}`)}</td><td>${escapeHtml(entry.confidence)}</td></tr>`)
        .join("");
    const symbols = model.facts.symbols
        .slice(0, 80)
        .map((symbol) => `<tr><td>${escapeHtml(symbol.kind)}</td><td>${escapeHtml(symbol.name)}</td><td>${escapeHtml(`${symbol.file}:${symbol.line}`)}</td></tr>`)
        .join("");
    return `<!doctype html>
<html lang="en">
<head>
...
export async function serveGagcodeHtml(model, indexes, port) {
